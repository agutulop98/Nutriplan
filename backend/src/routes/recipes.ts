...
const IngredientSchema = z.union([
  z.string().transform(name => ({ name, kcal: 0 })),
  z.object({ name: z.string(), kcal: z.number().int().min(0).default(0) }),
]);
...
async function buildRecipe(row: any) {
  const { rows: ings } = await pool.query(
    'SELECT ingredient, kcal FROM recipe_ingredients WHERE recipe_id = $1 ORDER BY sort_order',
    [row.id]
  );
  ...
  const ingredients = ings.map((r: any) => ({ name: r.ingredient, kcal: r.kcal ?? 0 }));
  const calcCalories = ingredients.reduce((sum: number, i: any) => sum + i.kcal, 0);
  return {
    ...row,
    calories: calcCalories > 0 ? calcCalories : row.calories,
    ingredients,
    steps: steps.map((r: any) => r.description),
  };
}
...
router.post('/', authenticate, async (req: AuthRequest, res) => {
  ...
  const { name, image, time, difficulty, category, servings, ingredients, steps } = parsed.data;
  const calcCalories = ingredients.reduce((sum, i) => sum + i.kcal, 0) || null;
  ...
  try {
    ...
    const result = await client.query(
      ...
      [name, image ?? null, time ?? null, calcCalories, difficulty ?? null, category ?? null, servings ?? 2]
    );
    const id = result.rows[0].id;
    for (let i = 0; i < ingredients.length; i++) {
      await client.query(
        'INSERT INTO recipe_ingredients (recipe_id, ingredient, kcal, sort_order) VALUES ($1,$2,$3,$4)',
        [id, ingredients[i].name, ingredients[i].kcal, i]
      );
    }
...
router.put('/:id', authenticate, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [name, image ?? null, time ?? null, calcCalories, difficulty ?? null, category ?? null, servings ?? 2, id]
    );
